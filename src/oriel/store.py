    ...
    def record(
        ...
        try:
            self.connection.execute(
                ...
                (tenant_id, task, case_id, model, prompt_version, input_hash, int(passed), latency_ms, cost_microusd),
            )
            self.connection.commit()
        except sqlite3.IntegrityError as error:
            raise ValueError("duplicate immutable trial") from error

    def aggregates(self, tenant_id: str, task: str, prompt_version: str) -> list[Aggregate]:
        rows = self.connection.execute(
            """SELECT model, SUM(passed), COUNT(*), AVG(latency_ms), AVG(cost_microusd)
               FROM trials WHERE tenant_id=? AND task=? AND prompt_version=? GROUP BY model""",
